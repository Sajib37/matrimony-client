...
const Login = () => {
    ...
    return (
        <section className="mt-8 min-h-screen flex items-center py-12 md:px-10 px-4 lg:px-0" style={{backgroundImage: `url(${bgImage})` ,backgroundSize:'cover'}}>
            ...
            <Toaster/>
            <section className="w-full max-w-screen-xl border-2 px-2 md:px-4 mx-auto md:py-2 py-2  flex items-center gap-2" style={{backgroundImage: `url(${bgImage})` , backgroundSize:'cover' ,boxShadow:"2px 2px 10px 1px #808080" ,border: "none"}}>
                ...
                <section className="w-full lg:w-1/2  mx-auto md:px-4  md:w-2/3 lg:max-w-lg rounded-lg py-6 md:py-4">
                    <h1 className="text-3xl text-black md:text-4xl mb-4 text-center font-semibold">
                    ...
                    <form action="" onSubmit={handleEmailLogin}>
                        ...
                        {/* captcha */}
                            <LoadCanvasTemplate/>
                        <div className="mb-1 mt-2 text-black">
                            <TextInput name='capctha'  ref={captchaRef}  id="input-gray" type='text' placeholder="Enter the text above" required color="gray"/>
                        </div>
                         <button onClick={handleValidate} className={`px-2 py-1 mb-2  text-black ${disabled? 'bg-[#D1A054]' : 'bg-green-600'}  rounded-lg`}>validate </button>
                        <p  onClick={handleResetPassword} className="mb-2 text-sm font-semibold hover:cursor-pointer text-red-600">Forgot password ?</p>
                        <button name='submit' disabled={disabled} className={`w-full font-semibold  py-2 rounded-lg ${disabled ? ' text-gray-800 bg-gray-300' :'text-black bg-[#D1A054]'}`}>Login</button>
                    </form>  
                    
                    <p className="font-ubuntu font-medium text-black mt-4">Don't have an account? Please, <Link className="text-[#D1A054]" to='/register'>Register</Link></p>
